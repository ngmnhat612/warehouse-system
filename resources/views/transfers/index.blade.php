    ...
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <form method="GET" action="{{ route('transfers.index') }}" class="d-flex gap-2 flex-wrap">
            ...
            <input type="date" class="form-control" name="date_from" value="{{ request('date_from') }}"
                style="width:140px">
            <input type="date" class="form-control" name="date_to" value="{{ request('date_to') }}" style="width:140px">
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php
                            $typeLabels = [1 => ['Sắp xếp kho', 'secondary'], 2 => ['Từ Quarantine', 'warning'], 3 =>
                            ['Khác', 'info']];
                            [$typeText, $typeColor] = $typeLabels[$transfer->transfer_type] ?? ['?', 'secondary'];
                            @endphp
                            <span
                                class="badge bg-{{ $typeColor }}-subtle text-{{ $typeColor }}-emphasis border border-{{ $typeColor }}-subtle rounded-pill">
                                {{ $typeText }}
                            </span>
                        </td>
                        ...
                        <td class="text-center">
                            <span
                                class="badge bg-primary-subtle text-primary-emphasis">{{ $transfer->details_count }}</span>
                        </td>
                        ...
                        <td>
                            ...
                            [$statusText, $statusClass] = $statusMap[$transfer->status] ?? ['?', 'bg-secondary-subtle
                    ...
                    <tr>
                        <td colspan="8" class="text-center text-body-secondary py-5">
                            <svg class="icon icon-3xl d-block mx-auto mb-2 opacity-25">
                                <use xlink:href="{{ asset('vendor/coreui/icons/sprites/free.svg#cil-transfer') }}">
                                </use>
                            </svg>
