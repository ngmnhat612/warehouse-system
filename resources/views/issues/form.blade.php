...
@endphp
  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      ...
      <small class="text-body-secondary">{{ $isEdit ? $issue->code : 'Điền thông tin và thêm hàng hóa cần xuất' }}</small>
    </div>
          ...
          <div class="card-body">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control text-uppercase @error('code') is-invalid @enderror"
                     name="code" value="{{ old('code', $issue->code ?? '') }}"
                     placeholder="VD: XK-2024-001" required maxlength="50"
                     {{ $isEdit ? 'readonly' : '' }}>
            ...
            <div class="mb-3">
              ...
              <select class="form-select @error('issue_type') is-invalid @enderror"
                ...
                <option value="1" {{ old('issue_type', $issue->issue_type ?? 1) == 1 ? 'selected' : '' }}>Sản xuất</option>
                <option value="2" {{ old('issue_type', $issue->issue_type ?? 1) == 2 ? 'selected' : '' }}>Bảo trì</option>
                <option value="3" {{ old('issue_type', $issue->issue_type ?? 1) == 3 ? 'selected' : '' }}>Mượn</option>
                <option value="4" {{ old('issue_type', $issue->issue_type ?? 1) == 4 ? 'selected' : '' }}>Khác</option>
              </select>
            ...
            <div class="mb-3" id="returnDateGroup" style="{{ old('issue_type', $issue->issue_type ?? 1) == 3 ? '' : 'display:none' }}">
...
let rowIndex = {{ $isEdit ? $issue->details->count() : 0 }};
...
function onProductChange(sel) {
  ...
  if (lotSel) {
    ...
    lotSel.innerHTML = '<option value="">— Không chọn —</option>' +
      productLots.map(l => `<option value="${l.id}">${l.lot_number}${l.expiry_date ? ' (HSD: ' + l.expiry_date + ')' : ''}</option>`).join('');
  }
