...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <form method="GET" action="{{ route('issues.index') }}" class="d-flex gap-2 flex-wrap">
            ...
            @if(request('search') || request('issue_type') || request('status') || request('date_from') ||
            request('date_to'))
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="small">
                            ...
                            @php $isOverdue = \Carbon\Carbon::parse($issue->expected_return_date)->isPast() &&
                            $issue->status != 4 @endphp
                            <span class="{{ $isOverdue ? 'text-danger fw-semibold' : 'text-body-secondary' }}">
                                ...
                                @if($isOverdue) <svg class="icon icon-sm">
                        ...
                        <td class="small text-body-secondary">{{ $issue->createdBy?->name ?? '—' }}</td>
                        <td class="text-center">
                            <a href="{{ route('issues.show', $issue->id) }}"
                            ...
                            @if($issue->status == 1)
                            <a href="{{ route('issues.edit', $issue->id) }}" class="btn btn-sm btn-outline-primary me-1"
                                title="Chỉnh sửa">
                                <svg class="icon">
                            ...
                            <button class="btn btn-sm btn-outline-danger"
                                onclick="confirmDelete({{ $issue->id }}, '{{ $issue->code }}')" title="Xóa">
                                <svg class="icon">
                    ...
                    <tr>
                        <td colspan="11" class="text-center text-body-secondary py-5">
                            <svg class="icon icon-3xl d-block mx-auto mb-2 opacity-25">
                                <use xlink:href="{{ asset('vendor/coreui/icons/sprites/free.svg#cil-arrow-top') }}">
                                </use>
                            </svg>
