    ...
    <div class="col-sm-6 col-lg-3">
      <div class="card border-start border-start-4 border-start-primary">
        <div class="card-body d-flex align-items-center gap-3">
          <svg class="icon icon-2xl text-primary"><use xlink:href="{{ asset('vendor/coreui/icons/sprites/free.svg#cil-inbox') }}"></use></svg>
          <div>
  ...
  <div class="card">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
      ...
      <form method="GET" action="{{ route('receipts.index') }}" class="d-flex gap-2 flex-wrap">
        ...
        @if(request('search') || request('receipt_type') || request('status') || request('date_from') || request('date_to'))
          ...
          <tbody>
            @forelse ($receipts as $index => $receipt)
              ...
              <tr>
                ...
                <td class="small">{{ $receipt->receipt_date ? \Carbon\Carbon::parse($receipt->receipt_date)->format('d/m/Y') : '—' }}</td>
                ...
                <td class="small text-body-secondary">{{ $receipt->creator?->name ?? '—' }}</td>
                <td class="text-center">
                  <a href="{{ route('receipts.show', $receipt->id) }}"
                  ...
                  @if($receipt->status == 1)
                    ...
                    <button class="btn btn-sm btn-outline-danger"
                            onclick="confirmDelete({{ $receipt->id }}, '{{ $receipt->code }}')"
                            title="Xóa">
